...
public class OpenALAudio implements Audio, Runnable {
    ...
    public Sound newSound(PCMFloatChannel channel, int type) {
        OpenALSound sound = new OpenALSound(channel, this.soundBufferSize, type);
    ...
    public Sound newSoundStream(PCMFloatChannel channel, int type) {
        OpenALSound sound = new OpenALSound(channel, type);
        this.sounds.add(sound);
        return sound;
    }
    ...
    public void update(double deltaTime, double[] volumes) {
        OpenALAudio.nextTime += deltaTime;
