...
public class Audio implements Runnable {
    ...
    private static final double LPS = 60.0;
    ...
    public Sound newSound(PCMFloatChannel channel, int type) {
        Sound sound = new Sound(channel, this.soundBufferSize, type);
    ...
    public Sound newSoundStream(String filename, int type) {
        ...
        return this.newSoundStream(channel, type);
    }
    
    public Sound newSoundStream(PCMFloatChannel channel, int type) {
        Sound sound = new Sound(channel, type);
        this.sounds.add(sound);
        return sound;
    }
    
    public void update(double deltaTime, double[] volumes) {
        Audio.nextTime += deltaTime;
    ...
    public void run() {
            while(running) {
                try {
                    ...
                    this.threadUpdate(this.deltaTime);
                    synchronized(this) {
                        this.busy = false;
                        this.wait();
                    }
    ...
    private void threadUpdate(double deltaTime) {
    ...
    private int fillBuffer(int buffer, Sound sound) {
    ...
    private void resumeSound(Sound sound) {
        int source = sound.getSource();
        if (source != -1) {
            AL10.alSourcef(source, AL10.AL_PITCH, (float) sound.getPitch());
            AL10.alSourcef(source, AL10.AL_GAIN, (float) sound.getVolume());
    ...
    private void pauseSound(Sound sound) {
        int source = sound.getSource();
        if (source != -1) {
    ...
    private void updateSound(Sound sound, double deltaTime) {
        int source = sound.getSource();
        
        Sound.Command nextCommand = sound.popNextCommant();        
        
        if (nextCommand == Sound.Command.PLAY) {
        ...
        } else if (nextCommand == Sound.Command.PAUSE) {
            sound.setIsPaused(true);
            
            this.pauseSound(sound);
        } else if (nextCommand == Sound.Command.RESUME) {
            sound.setIsPlaying(true);
        ...
        } else if (nextCommand == Sound.Command.STOP) {
        ...
        if (source != -1) {
            ...
            if (sound.isPlaying()) {
                ...
                AL10.alSourcef(source, AL10.AL_GAIN, (float) sound.getVolume()*(float)this.volumes[sound.type()]);
                AL10.alSourcef(source, AL10.AL_PITCH, (float) sound.getPitch());
            }
